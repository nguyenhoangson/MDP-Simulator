...
//Robot.getMaxSensorRange()
    ...
    public ArrayList<Integer> getMovement(List<PathFinder.Node> path, int direction) throws Exception {
        ...
        for (int i = 0; i < path.size() - 1; i++) {
            int moveAction = convertion(path.get(i), path.get(i + 1), direction);
    ...
    public int convertion(PathFinder.Node current, PathFinder.Node next, int direction) throws Exception {
        int action = 0;
        ...
        //System.out.println(direction + "n Y:" + nexY + "nX:" + nexX + " Y:" + curY + "X:" + curX);
        if (direction == NORTH) {
            if (nexY - curY == -1) {
                return MOVEFORWARD;
            } else if (nexY - curY == 1) {
